...
- Таблица кампаний с сортировкой по любой колонке; кампании кабинетов с
  ...
  подсвечиваются.
- Страница «График» — столбчатое сравнение аккаунтов и кампаний с переключателем
  метрики.
- Вкладка «ПЗ» — поисковые запросы (SEARCH_QUERY_PERFORMANCE_REPORT): кампания,
  запрос, расход, показы, клики, лиды, цена лида — с сортировкой и фильтром по
  тексту запроса.
- Вкладка «Аномалии» — ИИ-разбор отклонений (скачки расхода, рост CPA, риск
  остановки из-за низкого баланса, падение охватов и т.п.) через Google Gemini,
  по кнопке и не чаще раза в сутки; сравнивает период с предыдущим равным.
...
Один клиент может владеть несколькими кабинетами. Соответствие «кабинет →
клиент» задаётся **одной** переменной `YANDEX_CLIENTS` — по строке на кабинет
в формате `<id кабинета>=<Клиент>`:

```
YANDEX_CLIENTS="acc1=Лесорубы
acc2=Скуд
acc3=Лесорубы
acc4=Шеган"
```

Это включает фильтр «Клиент» на дашборде (при выборе клиента показываются все
его кабинеты) и колонку «Клиент» в таблице «По кабинетам». Список удобно хранить
отдельно и при новом клиенте просто вставлять обновлённую версию переменной.
`id кабинета` — это `<KEY>` из `YANDEX_ACCOUNT_<KEY>_TOKEN` (в нижнем регистре),
он же виден в колонке «ID».
...
| `GEMINI_API_KEY` | нет | Ключ Google AI Studio для вкладки «Аномалии». Без него вкладка показывает «не настроено». |
| `GEMINI_MODEL` | нет | Модель Gemini (по умолчанию `gemini-2.5-flash`). |
| `YANDEX_LEAD_GOALS` / `YANDEX_ACCOUNT_<KEY>_GOALS` | нет | ID целей Метрики для подсчёта «Лидов» (иначе считаются все конверсии). |
| `YANDEX_CLIENTS` | нет | Соответствие «кабинет → клиент» (по строке `id=Клиент`). Включает фильтр «Клиент». |
